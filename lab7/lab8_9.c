...
unsigned int CPU_fetchCode(char *mem, int codeOffset){
    ...
    // Pull one 32-bit instruction from the code section.
...
unsigned char CPU_Decode(unsigned int machineCode){
    ...
    // R-type code uses the function field instead.
    opcode = (unsigned char)((machineCode >> 26) & 0x3F);
    if (opcode == 0){
        opcode = (unsigned char)(machineCode & 0x3F);
    }
